...
export default function TestDetailPage() {
  ...
  const copyFixInstructions = () => {
    ...
    const getFixInstruction = (analysis: string) => {
      const parts = analysis.split('📋 Fix Instruction (copy-paste to your agent\'s system prompt):\n');
    ...
    const instructions = `🛡️ SECURITY HARDENING — Add these rules to your system prompt:
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-neutral-400">
                    Based on {benchmark.totalScans} scans • Average: {benchmark.avgScore}/100 • Median: {benchmark.medianScore}/100
                  </p>
                </div>
              </div>
            </div>
              ...
              <div key={result.id} className="border border-neutral-800 rounded-lg overflow-hidden">
                ...
                {isExpanded && (
                  <div className="p-4 border-t border-neutral-800 bg-neutral-950 space-y-4">
                    ...
                    {!result.passed && result.analysis?.includes('📋 Fix Instruction') && (
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-xs text-yellow-500 font-semibold">📋 Fix Instruction — Copy & paste to your agent</p>
                          <button
                            aria-label="Copy fix instruction"
                            onClick={(e) => {
                              e.stopPropagation();
                              const fix = result.analysis.split('📋 Fix Instruction (copy-paste to your agent\'s system prompt):\n')[1] || '';
                        ...
                        <div className="p-3 rounded bg-yellow-500/10 border border-yellow-500/30 font-mono">
                          <p className="text-sm text-yellow-200 whitespace-pre-wrap">
                            {result.analysis.split('📋 Fix Instruction (copy-paste to your agent\'s system prompt):\n')[1]?.trim()}
                          </p>
                        </div>
                      </div>
                    )}
        ...
        {!showDeleteConfirm ? (
          ...
          <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4">
            <p className="text-red-400 mb-4">
              ⚠️ This will permanently delete this test and all attack results. This cannot be undone.
            </p>
...
function generateMarkdownReport(
  ...
  let md = `# 🛡️ PwnClaw Security Report\n\n`;
  ...
  if (failed.length > 0) {
    ...
    for (const r of failed) {
      ...
      if (r.analysis?.includes('📋 Fix Instruction')) {
        const fix = r.analysis.split('📋 Fix Instruction (copy-paste to your agent\'s system prompt):\n')[1]?.trim();
  ...
  if (passed.length > 0) {
    ...
    for (const r of passed) {
      md += `- ✅ ${r.attackName} (${r.category})\n`;
    }
    md += `\n`;
  }
  
  md += `---\n*Generated by [PwnClaw](https://www.pwnclaw.com) — AI Agent Security Testing*\n`;
  return md;
}
